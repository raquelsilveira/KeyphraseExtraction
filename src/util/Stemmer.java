...
public class Stemmer {
	...
	public String stemmer(String expression) {
		util.Stemmer stem = new util.Stemmer();
		String stemmer = "";
		
		int index = 0;
		boolean begin = true;
		StringTokenizer tokenize = new StringTokenizer(expression, " +");
		while(tokenize.hasMoreTokens()) {
			...
			for (int i = 0; i < token.length(); i++)
				stem.add(token.charAt(i));
			stem.stem();
			stemmer += stem.toString() + (expression.length() > index ? expression.charAt(index) : "");
		}
